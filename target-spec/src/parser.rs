...
/// Parses this expression into a `TargetEnum` instance.
pub(crate) fn parse(input: &str) -> Result<TargetEnum, ParseError> {
    if input.starts_with("cfg(") {
        Ok(TargetEnum::Spec(Arc::new(
            Expression::parse(input).map_err(ParseError::new)?,
        )))
    } else {
        Ok(TargetEnum::TargetInfo(
            get_target_by_triple(input)
                .ok_or_else(|| ParseError(format!("unrecognized target triple '{}'", input)))?,
        ))
    }
}
