...
impl<'g, 'a, 'explain> fmt::Display for HakariExplainDisplay<'g, 'a, 'explain> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        // header row
        let row = Row::new()
            .with_cell("package".style(self.styles.header_style))
            .with_cell("include dev?".style(self.styles.header_style))
            .with_cell("features".style(self.styles.header_style))
            .with_cell("platform".style(self.styles.header_style));
        table.add_row(row);
            ...
            for (&features, inner) in explain_map {
                ...
                for package_id in package_set.package_ids(DependencyDirection::Reverse) {
                        ...
                        inner_value.sets.iter().enumerate()
                    {
                        ...
                        let mut row = Row::new();
                        if idx == 0 {
                            row.add_cell(&name_display);
                        } else {
                            row.add_cell(DITTO_MARK.style(self.styles.ditto_style));
                        }

                        row.add_cell(include_dev_display)
                            .add_cell(features_display)
                            .add_cell(platform_display);
                        table.add_row(row);
                    }
                }
                ...
                for (idx, platform_idx) in inner.fixup_platforms.iter().enumerate() {
                    let mut row = Row::new();
                    if idx == 0 {
                        row.add_cell("post-compute fixup");
                    } else {
                        row.add_cell(DITTO_MARK.style(self.styles.ditto_style));
                    }
                    ...
                    row.add_cell("-").add_cell("-").add_cell(platform_display);
                    table.add_row(row);
                }
            }
        }
