...
pub static HAKARI_CONFIG_PATH: &str = "Hakari.toml";
    ...
    fn exec(self, output: OutputOpts) -> Result<i32> {
        ...
        match self {
            Command::Initialize {
                ...
                let config_path = if skip_config {
                    None
                } else {
                    Some(Utf8Path::new(HAKARI_CONFIG_PATH))
                };

                let init =
                    HakariInit::new(&package_graph, package_name, &workspace_path, config_path)
...
fn config_path(package_graph: &PackageGraph) -> Utf8PathBuf {
    package_graph.workspace().root().join(HAKARI_CONFIG_PATH)
}
